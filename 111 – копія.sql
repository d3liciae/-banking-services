...
update тарифи_для_юридичних_осіб set максимальна_сума = 200000::money where ід = 3;

update тарифи_для_юридичних_осіб set мінімальна_сума = 1000000::money where ід = 4;
...
INSERT INTO тарифи_для_юридичних_осіб(назва, ід_валюти, відсотки_в_місяць_депозитні, відсотки_в_місяць_прострочені, відсотки_в_місяць_строкові, відсоток_комісії_зняття_коштів, відсоток_комісії_поповнення, мінімальна_сума, максимальна_сума) VALUES
('Кредитний gold', 1, NULL, 0.02, 0.02, 0.00, 0.00, 500001, 1000000),
('Кредитний silver', 1, NULL, 0.03, 0.03, 0.00, 0.00, 200001, 500000);
...
update рахунки_юридичних_осіб set тип_рахунку = '' where ід = 2;
...
DROP FUNCTION set_check_employee();
...
-----------------------------5---------------------------------
-- кількість карт клієнта
CREATE FUNCTION cards_count(client_name VARCHAR(55))
RETURNS INT AS $$
DECLARE
	count_cards INT;
BEGIN
	IF (SELECT ід FROM юридичні_особи WHERE повна_назва = client_name OR коротка_назва = client_name) IS NULL
		AND (SELECT ід FROM фізичні_особи WHERE ПІБ = client_name) IS NULL THEN
		RAISE EXCEPTION 'Даних про особу % у базі не зареєстровано', client_name
      			USING HINT = 'Будь ласка, перевірте дані!';
	ELSE
		count_cards = (SELECT count(карткові_рахунки.ід) 
					   FROM фізичні_особи INNER JOIN карткові_рахунки ON фізичні_особи.ід = карткові_рахунки.ід_фізичної_особи
					   AND фізичні_особи.ПІБ = client_name);
		IF count_cards IS NULL THEN
			count_cards = (SELECT count(рахунки_юридичних_осіб.ід) 
					       FROM юридичні_особи INNER JOIN рахунки_юридичних_осіб ON юридичні_особи.ід = рахунки_юридичних_осіб.ід_.юридичної_особи
					  	   WHERE юридичні_особи.повна_назва = client_name 
						   OR юридичні_особи.коротка_назва = client_name);
			RETURN count_cards;
		END IF;
		RETURN count_cards;
	END IF;
...
DROP FUNCTION cards_count;

SELECT * FROM cards_count('Торговельна організація "Івано-Франківськ"');
