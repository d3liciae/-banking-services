...
SELECT * FROM
	(SELECT підрозділи.назва AS підрозділи, штатний_розпис.назва_посади AS посади, 
		...
		rank() OVER (PARTITION BY підрозділи.назва ORDER BY COUNT(працівники.ід) DESC)
...
BEGIN
	...
	currensy_id = (SELECT ід_валюти FROM тарифи_для_фізичних_осіб WHERE ід = NEW.ід_картки_клієнта);
